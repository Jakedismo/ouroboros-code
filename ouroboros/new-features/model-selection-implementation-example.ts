...
class ModelRegistry {
  private models = new Map<LLMProvider, ModelInfo[]>();
  private currentModels = new Map<LLMProvider, string>();
  private equivalencies = new Map<string, Map<LLMProvider, string>>();
  ...
  async getAvailableModels(provider: LLMProvider): Promise<ModelInfo[]> {
  ...
  async setModel(provider: LLMProvider, modelId: string): Promise<void> {
...
// ============================================================================
// 2. Model Command Parser Implementation Example
// ============================================================================
...
class ModelCommandParser {
  ...
  private parseProviderSpecific(input: string): ModelCommandRequest {
    const parts = input.split(/\s+/);
    const requests: ModelCommandRequest[] = [];
  ...
  private parseProvider(str: string): LLMProvider | null {
    const normalized = str.toLowerCase();

    if (normalized === 'gemini') return LLMProvider.GEMINI;
  ...
  private parseListCommand(input: string): ModelCommandRequest {
    const parts = input.split(/\s+/);
    if (parts.length > 1) {
  ...
  private parseResetCommand(input: string): ModelCommandRequest {
    const parts = input.split(/\s+/);
    if (parts.length > 1) {
      const provider = this.parseProvider(parts[1]);
      if (provider) {
  ...
  private parseGlobalCommand(input: string): ModelCommandRequest {
...
class RuntimeModelManager {
  ...
  private providers: Map<LLMProvider, any>; // Provider instances
  private config: ModelConfigPersistence;
  ...
  async switchModel(
    ...
    // 1. Validate model
    const isAvailable = await this.registry.isModelAvailable(
      provider,
      newModel,
    );
    if (!isAvailable) {
  ...
  async getCurrentConfiguration(): Promise<Map<LLMProvider, ModelInfo>> {
    const config = new Map<LLMProvider, ModelInfo>();
...
class ModelCommandExecutor {
  private parser: ModelCommandParser;
  private manager: RuntimeModelManager;
  private ui: ModelCommandUI;
  ...
  private async handleSet(request: ModelCommandRequest) {
    if (request.scope === 'all' && request.model) {
      ...
      const result = await this.manager.switchModel(
        request.provider,
        request.model,
      );
  ...
  private async handleList(request: ModelCommandRequest) {
    if (request.provider) {
      // List for specific provider
      const models = await this.manager.registry.getAvailableModels(
        request.provider,
      );
  ...
  private async handleGet(request: ModelCommandRequest) {
    const config = await this.manager.getCurrentConfiguration();
    this.ui.showCurrentConfiguration(config);
  }

  private async handleReset(request: ModelCommandRequest) {
...
class ModelCommandUI {
  showSwitchResult(provider: LLMProvider, result: SwitchResult) {
    if (result.success) {
  ...
  showBatchResults(results: Map<LLMProvider, SwitchResult>) {
  ...
  showProviderModels(provider: LLMProvider, models: ModelInfo[]) {
  ...
  showAllModels(registry: ModelRegistry) {
  ...
  showCurrentConfiguration(config: Map<LLMProvider, ModelInfo>) {
    ...
    for (const [provider, model] of config) {
      const context = this.formatTokens(model.capabilities.contextWindow);
      console.log(
        `║ ${provider.padEnd(9)} │ ${model.id.padEnd(15)} │ ${context.padEnd(8)} ║`,
      );
    }
...
class ModelConfigPersistence {
  ...
  async saveModel(provider: LLMProvider, model: string): Promise<void> {
    if (!this.config) {
    ...
    if (this.config.history.enabled) {
      ...
      if (
        this.config.history.recentModels.length >
...
interface ModelSelectionConfig {
  ...
  aliases: Record<string, { provider: LLMProvider; model: string }>;
  history: {
    enabled: boolean;
    recentModels: Array<{
      provider: LLMProvider;
