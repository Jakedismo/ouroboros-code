...
To start the Ouroboros Code from the source code (after building), run the following command from the root directory:
...
The integration tests are designed to validate the end-to-end functionality of the Ouroboros Code. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for the Ouroboros Code.
...
1.  **Start the Ouroboros Code in development mode:**
...
Container-based sandboxing mounts the project directory (and system temp directory) with read-write access and is started/stopped/removed automatically as you start/stop Ouroboros Code. Files created within the sandbox should be automatically mapped to your user/group on host machine. You can easily specify additional mounts, ports, or environment variables by setting `SANDBOX_{MOUNTS,PORTS,ENV}` as needed. You can also fully customize the sandbox for your projects by creating the files `.gemini/sandbox.Dockerfile` and/or `.gemini/sandbox.bashrc` under your project settings directory (`.gemini`) and running `gemini` with `BUILD_SANDBOX=1` to trigger building of your custom sandbox.
