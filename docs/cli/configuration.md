...
- **Location:** `~/.ouroboros/tmp/<project_hash>/shell_history` (legacy installs continue to support `~/.gemini/tmp/...`).
...
**Environment Variable Exclusion:** Some environment variables (like `DEBUG` and `DEBUG_MODE`) are automatically excluded from being loaded from project `.env` files to prevent interference with Ouroboros Code behavior. Variables from `.ouroboros/.env` (or the legacy `.gemini/.env`) files are never excluded. You can customize this behavior using the `advanced.excludedEnvVars` setting in your `settings.json` file.
...
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  ...
  - Example: `ouroboros-code -i "explain this code"`
- **`--sandbox`** (**`-s`**):
