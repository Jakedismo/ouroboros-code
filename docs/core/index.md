...
Ouroboros Code's core package (`packages/core`) is the backend portion of Ouroboros Code, handling communication with the Gemini API, managing tools, and processing requests sent from `packages/cli`. For a general overview of Ouroboros Code, see the [main documentation page](../index.md).
...
While the `packages/cli` portion of Ouroboros Code provides the user interface, `packages/core` is responsible for:
...
Ouroboros Code includes a model fallback mechanism to ensure that you can continue to use the CLI even if the default "pro" model is rate-limited.
