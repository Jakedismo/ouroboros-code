...
let logger;
let log = () => { };
function updateWorkspacePath(context) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (workspaceFolders && workspaceFolders.length > 0) {
        const workspacePaths = workspaceFolders
            .map((folder) => folder.uri.fsPath)
            .join(path.delimiter);
        context.environmentVariableCollection.replace(IDE_WORKSPACE_PATH_ENV_VAR, workspacePaths);
    }
    else {
        context.environmentVariableCollection.replace(IDE_WORKSPACE_PATH_ENV_VAR, '');
    }
}
