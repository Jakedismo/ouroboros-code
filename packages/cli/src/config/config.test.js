...
import { ShellTool, EditTool, WriteFileTool } from '@ouroboros/code-cli-core';
import { loadCliConfig, parseArguments } from './config.js';
import * as ServerConfig from '@ouroboros/code-cli-core';
...
vi.mock('@ouroboros/code-cli-core', async () => {
    const actualServer = await vi.importActual('@ouroboros/code-cli-core');
    return {
        ...actualServer,
        IdeClient: {
...
describe('Approval mode tool exclusion logic', () => {
    ...
    it('should merge approval mode exclusions with settings exclusions in auto_edit mode', async () => {
        ...
        const argv = await parseArguments();
        const settings = { excludeTools: ['custom_tool'] };
        const extensions = [];
        const config = await loadCliConfig(settings, extensions, 'test-session', argv);
        const excludedTools = config.getExcludeTools();
...
describe('loadCliConfig folderTrustFeature', () => {
    ...
    it('should be true when settings.folderTrustFeature is true', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { folderTrustFeature: true };
        const config = await loadCliConfig(settings, [], 'test-session', argv);
...
describe('loadCliConfig folderTrust', () => {
    ...
    it('should be false if folderTrustFeature is true and folderTrust is false', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { folderTrustFeature: true, folderTrust: false };
        const config = await loadCliConfig(settings, [], 'test-session', argv);
        expect(config.getFolderTrust()).toBe(false);
    });
    it('should be false if folderTrustFeature is false and folderTrust is true', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { folderTrustFeature: false, folderTrust: true };
        const config = await loadCliConfig(settings, [], 'test-session', argv);
        expect(config.getFolderTrust()).toBe(false);
    });
    it('should be true when folderTrustFeature is true and folderTrust is true', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { folderTrustFeature: true, folderTrust: true };
        const config = await loadCliConfig(settings, [], 'test-session', argv);
...
describe('loadCliConfig trustedFolder', () => {
    ...
    const testCases = [
        // Cases where folderTrustFeature is false (feature disabled)
        {
            ...
            description: 'feature disabled, folderTrust true, workspace trusted -> behave as trusted',
        },
        {
            folderTrustFeature: false,
            folderTrust: true,
            ...
            expectedFolderTrust: false,
            expectedIsTrustedFolder: true,
            description: 'feature disabled, folderTrust true, workspace not trusted -> behave as trusted',
        },
        {
            folderTrustFeature: false,
            folderTrust: false,
            ...
            expectedFolderTrust: false,
            expectedIsTrustedFolder: true,
            description: 'feature disabled, folderTrust false, workspace trusted -> behave as trusted',
        },
        // Cases where folderTrustFeature is true but folderTrust setting is false
        {
            ...
            expectedFolderTrust: false,
            expectedIsTrustedFolder: true,
            description: 'feature on, folderTrust false, workspace trusted -> behave as trusted',
        },
        {
            folderTrustFeature: true,
            folderTrust: false,
            ...
            expectedFolderTrust: false,
            expectedIsTrustedFolder: true,
            description: 'feature on, folderTrust false, workspace not trusted -> behave as trusted',
        },
        // Cases where feature is fully enabled (folderTrustFeature and folderTrust are true)
        {
            ...
            expectedIsTrustedFolder: true,
            description: 'feature on, folderTrust on, workspace trusted -> is trusted',
        },
        {
            folderTrustFeature: true,
            folderTrust: true,
            ...
            description: 'feature on, folderTrust on, workspace NOT trusted -> is NOT trusted',
        },
        {
            folderTrustFeature: true,
            folderTrust: true,
            ...
            description: 'feature on, folderTrust on, workspace trust unknown -> is unknown',
        },
    ];
    for (const { folderTrustFeature, folderTrust, isWorkspaceTrusted: mockTrustValue, expectedFolderTrust, expectedIsTrustedFolder, description, } of testCases) {
        it(`should be correct for: ${description}`, async () => {
            isWorkspaceTrusted.mockImplementation((settings) => {
                const featureIsEnabled = (settings.folderTrustFeature ?? false) &&
                    (settings.folderTrust ?? true);
                return featureIsEnabled ? mockTrustValue : true;
            });
            const argv = await parseArguments();
            const settings = { folderTrustFeature, folderTrust };
            const config = await loadCliConfig(settings, [], 'test-session', argv);
