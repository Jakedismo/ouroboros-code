...
import { Config, MCPServerConfig } from '@ouroboros/code-cli-core';
import {
  MultiProviderMCPConfig,
  MultiProviderMCPConfigMerger,
  MultiProviderMCPConfigValidator,
...
export function configureMCPForProvider(
    ...
    case 'gemini': {
      ...
      if (geminiSettings) {
        setProviderToolSettings(config, 'gemini', {
          functionCallingMode:
            geminiSettings.functionCallingMode ?? 'FUNCTION_CALLING',
  ...
  setSharedToolExecutionSettings(config, mergedConfig.toolExecution as unknown as Record<string, unknown>);
 ...
 */
function setProviderToolSettings(
  config: Config,
  provider: string,
 ...
 */
function setSharedToolExecutionSettings(
  config: Config,
 ...
 */
function setMCPServerConfigurations(
  config: Config,
 ...
 */
export function validateCLIMCPConfig(config: CLIMCPConfig): string[] {
  const errors: string[] = [];
 ...
 */
export function getEnabledProviders(config: CLIMCPConfig): CLIProviderType[] {
  return Object.entries(config.providers)
    .filter(([_, providerConfig]) => providerConfig.enabled)
    .map(([providerName]) => providerName as CLIProviderType);
}
 ...
 */
export function getMCPEnabledProviders(
  config: CLIMCPConfig,
): CLIProviderType[] {
  return Object.entries(config.providers)
    .filter(
      ([_, providerConfig]) =>
        providerConfig.enabled && providerConfig.mcpEnabled,
    )
    .map(([providerName]) => providerName as CLIProviderType);
}
 ...
 * @param config CLI MCP configuration.
 */
export function displayConfigSummary(config: CLIMCPConfig): void {
  ...
  for (const [name, providerConfig] of Object.entries(config.providers)) {
    console.log(
      `  ${name}: enabled=${providerConfig.enabled}, mcp=${providerConfig.mcpEnabled}`,
    );
  }
  ...
  console.log(
    `  Confirmation Mode: ${config.mcpConfig.toolExecution.confirmationMode}`,
  );
  console.log(
    `  Max Concurrent Tools: ${config.mcpConfig.toolExecution.maxConcurrentTools}`,
  );
...
export const CLI_MCP_USAGE = {
  ...
  flags: {
    ...
    '--max-concurrent-tools':
      'Set maximum number of concurrent tool executions',
