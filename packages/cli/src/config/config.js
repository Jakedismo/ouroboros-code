...
Autonomous agent: ouroboros-code --prompt "continue autonomously"`)
        ...
        .option('tool-timeout', {
        type: 'number',
        description: 'Set tool execution timeout in milliseconds (default: 60000)',
        default: 60000,
    })
        .option('max-concurrent-tools', {
        type: 'number',
        description: 'Maximum number of concurrent tool executions (default: 3)',
        default: 3,
    })
...
export async function loadCliConfig(settings, extensions, sessionId, argv, cwd = process.cwd()) {
    ...
    const multiProviderMCPConfig = {};
    if (argv.toolTimeout !== undefined || argv.maxConcurrentTools !== undefined || argv.confirmationMode !== undefined) {
        // Only specify the toolExecution settings that are overridden via CLI
        // The Config class will merge these with defaults
        const toolExecutionOverrides = {};
        if (argv.toolTimeout !== undefined) {
            toolExecutionOverrides['timeoutMs'] = argv.toolTimeout;
        }
        if (argv.maxConcurrentTools !== undefined) {
            toolExecutionOverrides['maxConcurrentTools'] = argv.maxConcurrentTools;
        }
        if (argv.confirmationMode !== undefined) {
            toolExecutionOverrides['confirmationMode'] = argv.confirmationMode;
        }
        if (Object.keys(toolExecutionOverrides).length > 0) {
            multiProviderMCPConfig.toolExecution = toolExecutionOverrides;
        }
    }
    return new Config({
        sessionId,
