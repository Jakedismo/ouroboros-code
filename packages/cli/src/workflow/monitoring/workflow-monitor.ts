...
export interface WorkflowStep {
  ...
  name: string;
  description: string;
  command: string; // e.g., "/apple-control mail:read count=10"
  parameters: Record<string, any>;
  estimatedDuration: number; // milliseconds
  dependencies?: string[]; // step IDs that must complete first
  parallel?: boolean; // can run in parallel with other steps
  retryCount?: number; // number of retry attempts allowed
...
export interface WorkflowDefinition {
  ...
  asciiDiagram?: string; // the generated ASCII workflow diagram
}
...
export class WorkflowMonitor extends EventEmitter {
  ...
  private executionHistory = new Map<string, WorkflowExecutionState>();
   ...
   */
  async startWorkflowMonitoring(definition: WorkflowDefinition): Promise<WorkflowExecutionState> {
  ...
  completeStepExecution(
    workflowId: string, 
    stepId: string, 
    result: Omit<StepExecutionResult, 'stepId'>
  ): void {
    const executionState = this.activeWorkflows.get(workflowId);
    if (!executionState) {
   ...
   */
  getExecutionHistory(limit: number = 50): WorkflowExecutionState[] {
   ...
   */
  private updatePerformanceMetrics(executionState: WorkflowExecutionState): void {
 ...
 */
let globalWorkflowMonitor: WorkflowMonitor | null = null;
 ...
 */
export async function initializeWorkflowMonitoring(): Promise<WorkflowMonitor> {
