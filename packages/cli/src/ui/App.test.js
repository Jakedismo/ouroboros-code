...
import { Config as ServerConfig, ApprovalMode, ideContext, } from '@ouroboros/code-cli-core';
...
// Mock @ouroboros/code-cli-core and its Config class
...
const { isGitRepository: mockedIsGitRepository } = vi.mocked(await import('@ouroboros/code-cli-core'));
...
describe('App UI', () => {
    ...
    let mockVersion;
    let currentUnmount;
    const createMockSettings = (settings = {}, isTrusted = true) => {
        ...
        return new LoadedSettings(systemSettingsFile, userSettingsFile, workspaceSettingsFile, [], isTrusted);
    };
    beforeEach(() => {
    ...
    describe('handleAutoUpdate', () => {
        ...
        it('should not start the update process when running from git', async () => {
            ...
            const info = {
                update: {
                    name: '@ouroboros/multi-agent-cli',
                    latest: '1.1.0',
                    current: '1.0.0',
                },
            ...
            const { spawn } = await import('node:child_process');
            const { unmount } = renderWithProviders(_jsx(App, { config: mockConfig, version: mockVersion }), mockSettings);
        ...
        it('should show a success message when update succeeds', async () => {
            ...
            mockedCheckForUpdates.mockResolvedValue(info);
            const { lastFrame, unmount } = renderWithProviders(_jsx(App, { config: mockConfig, version: mockVersion }), mockSettings);
            currentUnmount = unmount;
            updateEventEmitter.emit('update-success', info);
            await new Promise((resolve) => setTimeout(resolve, 10));
            expect(lastFrame()).toContain('Update successful! The new version will be used on your next run.');
        });
        it('should show an error message when update fails', async () => {
            ...
            await new Promise((resolve) => setTimeout(resolve, 10));
            expect(lastFrame()).toContain('Automatic update failed. Please try updating manually');
        });
    ...
    it('should display active file and other context', async () => {
        ...
        currentUnmount = unmount;
        await Promise.resolve();
        expect(lastFrame()).toContain('Using: 1 open file (ctrl+g to view) | 1 GEMINI.md file');
    });
    ...
    it('should not display Footer component when hideFooter is true', async () => {
        ...
        const { lastFrame, unmount } = renderWithProviders(_jsx(App, { config: mockConfig, settings: mockSettings, version: mockVersion }));
        currentUnmount = unmount;
        await Promise.resolve();
    ...
    describe('with initial prompt from --prompt-interactive', () => {
        it('should submit the initial prompt automatically', async () => {
            ...
            const { unmount, rerender } = renderWithProviders(_jsx(App, { config: mockConfig, version: mockVersion }), mockSettings);
            currentUnmount = unmount;
            // Force a re-render to trigger useEffect
            rerender(_jsx(App, { config: mockConfig, version: mockVersion }));
    ...
    describe('Message Queuing', () => {
        ...
        it('should queue messages when handleFinalSubmit is called during streaming', () => {
            ...
            const { unmount } = renderWithProviders(_jsx(App, { config: mockConfig, settings: mockSettings, version: mockVersion }));
            currentUnmount = unmount;
        ...
        it('should auto-send queued messages when transitioning from Responding to Idle', async () => {
            ...
            const { unmount, rerender } = renderWithProviders(_jsx(App, { config: mockConfig, settings: mockSettings, version: mockVersion }));
            currentUnmount = unmount;
            ...
            rerender(_jsx(App, { config: mockConfig, settings: mockSettings, version: mockVersion }));
        ...
        it('should display queued messages with dimmed color', () => {
            ...
            const { unmount, lastFrame } = renderWithProviders(_jsx(App, { config: mockConfig, settings: mockSettings, version: mockVersion }));
            currentUnmount = unmount;
