...
import { getErrorMessage, getAllGeminiMdFilenames, ApprovalMode, isEditorAvailable, FlashFallbackEvent, logFlashFallback, AuthType, ideContext, } from '@ouroboros/code-cli-core';
...
import { isProQuotaExceededError, isGenericQuotaExceededError, UserTierId, } from '@ouroboros/code-cli-core';
...
export const AppWrapper = (props) => {
    ...
    return (_jsx(KeypressProvider, { kittyProtocolEnabled: kittyProtocolStatus.enabled, config: props.config, children: _jsx(SessionStatsProvider, { children: _jsx(VimModeProvider, { settings: settings, children: _jsx(App, { ...props }) }) }) }));
};
const App = ({ config, startupWarnings = [], version }) => {
    ...
    const { isThemeDialogOpen, openThemeDialog, handleThemeSelect, handleThemeHighlight, } = useThemeCommand(setThemeError, addItem);
    ...
    const { isEditorDialogOpen, openEditorDialog, handleEditorSelect, exitEditorDialog, } = useEditorSettings(setEditorError, addItem);
