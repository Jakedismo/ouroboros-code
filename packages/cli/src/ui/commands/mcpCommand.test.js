...
import { MCPServerStatus, MCPDiscoveryState, getMCPServerStatus, getMCPDiscoveryState, DiscoveredMCPTool, } from '@ouroboros/code-cli-core';
...
describe('mcpCommand', () => {
    ...
    describe('auth subcommand', () => {
        ...
        it('should authenticate with a specific server', async () => {
            ...
            const { MCPOAuthProvider } = await import('@ouroboros/code-cli-core');
            const authCommand = mcpCommand.subCommands?.find((cmd) => cmd.name === 'auth');
            const result = await authCommand.action(context, 'test-server');
            expect(MCPOAuthProvider.authenticate).toHaveBeenCalledWith('test-server', { enabled: true }, 'http://localhost:3000');
        ...
        it('should handle authentication errors', async () => {
            ...
            const { MCPOAuthProvider } = await import('@ouroboros/code-cli-core');
            MCPOAuthProvider.authenticate.mockRejectedValue(new Error('Auth failed'));
            const authCommand = mcpCommand.subCommands?.find((cmd) => cmd.name === 'auth');
            const result = await authCommand.action(context, 'test-server');
            expect(isMessageAction(result)).toBe(true);
            if (isMessageAction(result)) {
