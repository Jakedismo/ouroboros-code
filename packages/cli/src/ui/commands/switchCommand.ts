...
export const switchCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<MessageActionReturn> => {
    ...
    return await switchToProvider(config, argsArray[0]);
  },
 ...
 */
function showCurrentProvider(config: Config): MessageActionReturn {
 ...
 */
function listProviders(): MessageActionReturn {
  return {
    type: 'message',
    messageType: 'info',
 ...
 */
async function switchToProvider(config: Config, provider: string): Promise<MessageActionReturn> {
  ...
  // Actually switch the provider - MUST await async setProvider!
  await config.setProvider(normalizedProvider as 'gemini' | 'openai' | 'anthropic');
