...
import { WorkflowProgressDisplay, useWorkflowProgressDisplay, ProgressFormat } from './WorkflowProgressDisplay.js';
 ...
 */
export const WorkflowProgressOverlay: React.FC<WorkflowProgressOverlayProps> = ({
  position = 'center',
  dismissible = true,
  onDismiss,
  showHelp = true
  ...
  const renderStatsSummary = () => (
    <Box flexDirection={isNarrow ? 'column' : 'row'} marginBottom={1} gap={2}>
      <Box flexDirection="column">
        <Text color={Colors.AccentPurple} bold>📈 Statistics</Text>
        <Text color={Colors.Gray}>
          Active: {activeWorkflowsCount} | Completed: {completedWorkflowsCount}/{totalWorkflowsCount}
        </Text>
        <Text color={Colors.Gray}>
          Success Rate: {(overallSuccessRate * 100).toFixed(1)}% | Errors: {currentErrorCount}
        </Text>
      </Box>
      
      {!isNarrow && (
        <Box flexDirection="column">
          <Text color={Colors.AccentBlue} bold>⏱️ Performance</Text>
          <Text color={Colors.Gray}>
  ...
  const renderKeyboardHelp = () => (
    <Box flexDirection="column" marginTop={1} borderStyle="round" borderColor={Colors.Gray}>
      <Text color={Colors.AccentPurple} bold>🎮 Keyboard Controls</Text>
      <Box flexDirection={isNarrow ? 'column' : 'row'} gap={4}>
        <Box flexDirection="column">
          <Text color={Colors.Gray}>• <Text color={Colors.Foreground}>ESC/Q</Text> - Close overlay</Text>
          <Text color={Colors.Gray}>• <Text color={Colors.Foreground}>C</Text> - Toggle compact mode</Text>
          <Text color={Colors.Gray}>• <Text color={Colors.Foreground}>F</Text> - Change format</Text>
        </Box>
        <Box flexDirection="column">
          <Text color={Colors.Gray}>• <Text color={Colors.Foreground}>D</Text> - Toggle dashboard</Text>
          <Text color={Colors.Gray}>• <Text color={Colors.Foreground}>W</Text> - Open from mini</Text>
        </Box>
      </Box>
    </Box>
  );
  ...
  const renderHeader = () => (
    <Box marginBottom={1} justifyContent="space-between">
      ...
      <Text color={Colors.Gray}>
        Format: {format.replace('_', ' ').toUpperCase()} | {compact ? 'Compact' : 'Full'}
      </Text>
    </Box>
  );
  ...
  const renderDashboard = () => {
    ...
    return (
      <Box flexDirection="column" borderStyle="double" borderColor={Colors.AccentPurple} padding={1}>
        <Text color={Colors.AccentPurple} bold>📊 Live Dashboard</Text>
        <Box flexDirection="column">
          ...
          <Box marginTop={1}>
            <Text color={Colors.Gray}>Waiting for workflow execution to begin...</Text>
          </Box>
        </Box>
      </Box>
    );
...
export const WorkflowProgressMini: React.FC = () => {
  ...
  const statusColor = currentErrorCount > 0 ? Colors.AccentRed : Colors.AccentGreen;
