...
export interface WorkflowProgressActions {
  ...
  generateWorkflowVisualization: (workflowId: string, format?: ProgressFormat) => string;
}

/**
 * Combined workflow progress context
 */
export type WorkflowProgressContextType = WorkflowProgressState & WorkflowProgressActions;
...
export const WorkflowProgressProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleProgressUpdate = (update: ProgressUpdate) => {
      setState(prevState => {
        ...
        const completedCount = update.status === WorkflowStatus.COMPLETED ? 
          prevState.completedWorkflowsCount + 1 : prevState.completedWorkflowsCount;
        
        const totalCount = prevState.totalWorkflowsCount + 
          (prevState.activeWorkflows.has(update.workflowId) ? 0 : 1);

        const totalErrors = allUpdates.reduce((sum, u) => sum + u.performance.errorCount, 0);
        const avgSuccessRate = allUpdates.length > 0 ? 
          allUpdates.reduce((sum, u) => sum + u.performance.successRate, 0) / allUpdates.length : 1.0;
        
        const avgCompletionTime = allUpdates.length > 0 ?
          allUpdates.reduce((sum, u) => sum + u.overall.elapsedTime, 0) / allUpdates.length : 0;
  ...
  const setAutoHideDelay = useCallback((autoHideDelay: number) => {
    setState(prev => ({ ...prev, autoHideDelay: Math.max(0, autoHideDelay) }));
  }, []);

  // Progress tracking control actions
  const startTrackingWorkflow = useCallback((workflowId: string, updateInterval = 1000) => {
    if (progressTracker) {
      progressTracker.startTracking(workflowId, updateInterval);
    }
  }, [progressTracker]);

  const stopTrackingWorkflow = useCallback((workflowId: string) => {
    if (progressTracker) {
  ...
  const generateWorkflowVisualization = useCallback((workflowId: string, format?: ProgressFormat): string => {
    if (!progressTracker) return 'Progress tracker not initialized';
    return progressTracker.generateProgressVisualization(workflowId, format || state.format);
...
export const useWorkflowProgress = (): WorkflowProgressContextType => {
  ...
  if (!context) {
    throw new Error('useWorkflowProgress must be used within a WorkflowProgressProvider');
  }
  return context;
};
...
export const useWorkflowProgressStats = () => {
  ...
  const completionRate = totalWorkflowsCount > 0 ? completedWorkflowsCount / totalWorkflowsCount : 0;
