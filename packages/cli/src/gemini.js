...
import { sessionId, logUserPrompt, AuthType, getOauthClient, logIdeConnection, IdeConnectionEvent, IdeConnectionType, } from '@ouroboros/code-cli-core';
...
function getNodeMemoryArgs(config) {
    ...
    if (process.env['OUROBOROS_CLI_NO_RELAUNCH']) {
        return [];
    }
...
async function relaunchWithAdditionalArgs(additionalArgs) {
    ...
    const newEnv = { ...process.env, OUROBOROS_CLI_NO_RELAUNCH: 'true' };
...
export async function main() {
    ...
    if (config.isInteractive()) {
        ...
        const instance = render(_jsx(MainComponent, { initialConfig: config, settings: settings, startupWarnings: startupWarnings, version: version, workspaceRoot: workspaceRoot, extensions: extensions, argv: argv }), {
            exitOnCtrlC: false,
        });
        checkForUpdates()
            .then((info) => {
...
function setWindowTitle(title, settings) {
    if (!settings.merged.hideWindowTitle) {
        const windowTitle = (process.env['CLI_TITLE'] || `Ouroboros - ${title}`).replace(
