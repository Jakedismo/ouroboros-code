...
import { AdversarialChallenger, formatChallengeReport, } from '@ouroboros/code-cli-core';
/**
...
export class ChallengeCommand {
    _orchestrator;
    name = 'challenge';
    description = 'Run adversarial challenges where providers critique each other';
    kind = CommandKind.BUILT_IN;
    challenger;
    constructor(_orchestrator) {
        this._orchestrator = _orchestrator;
        this.challenger = new AdversarialChallenger(this._orchestrator);
    }
    action = async (_context, argsStr) => {
        ...
        try {
            ...
            return {
                type: 'message',
                messageType: 'error',
                content: `❌ Challenge failed: ${error.message}`,
            };
        }
    };
    /**
...
💡 **Tip**: Use --focus to concentrate critique on specific aspects (logic, facts, completeness, assumptions, practical)`;
     ...
     */
    generateTakeaway(critiqueCounts, _report) {
        const total = critiqueCounts.critical + critiqueCounts.moderate + critiqueCounts.minor;
...
//# sourceMappingURL=challenge-command.js.map