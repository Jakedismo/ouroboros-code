...
import { BlindspotDetector, formatBlindspotAnalysis, } from '@ouroboros/code-cli-core';
/**
...
export class BlindspotCommand {
    ...
    action = async (_context, argsStr) => {
        ...
        try {
            ...
            const analysis = await this.detector.detectBlindspots(options.query, options.providers);
            ...
            return {
                type: 'message',
                messageType: 'error',
                content: `❌ Blindspot analysis failed: ${error.message}`,
            };
        }
    };
    /**
    ...
    parseArguments(args) {
        ...
        while (i < args.length) {
            ...
            if (arg === '--providers' && i + 1 < args.length) {
                ...
                options.providers = providerList;
                i += 2;
            }
        ...
        if (!options.query) {
            throw new Error('No query provided. Usage: /blindspot "your question or input"');
        }
        return options;
    }
    /**
...
//# sourceMappingURL=blindspot-command.js.map