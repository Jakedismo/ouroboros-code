...
import { z } from 'zod';
import { Config, Storage } from '@ouroboros/code-cli-core';
  ...
  private async parseAndAdaptFile(
    ...
    return {
      ...
      action: async (
        ...
        console.log(`[FileCommandLoader] Executing custom command: ${baseCommandName}`);
        console.log(`[FileCommandLoader] Raw prompt: ${validDef.prompt}`);
        ...
        try {
          ...
          console.log(`[FileCommandLoader] Processing prompt with ${processors.length} processors`);
          for (const processor of processors) {
            processedPrompt = await processor.process(processedPrompt, context);
          }
          
          console.log(`[FileCommandLoader] Returning processed prompt: ${processedPrompt.substring(0, 100)}...`);
