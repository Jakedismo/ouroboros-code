...
import { Storage } from '@ouroboros/code-cli-core';
...
describe('FileCommandLoader', () => {
    ...
    beforeEach(() => {
        ...
        mockShellProcess.mockImplementation((prompt, context) => {
            ...
            const processedPrompt = prompt.replaceAll(SHORTHAND_ARGS_PLACEHOLDER, userArgsRaw);
    ...
    describe('Shell Processor Integration', () => {
        ...
        it('assembles the processor pipeline in the correct order (Shell -> Default)', async () => {
            ...
            mockShellProcess.mockImplementation((p) => Promise.resolve(`${p}-shell-processed`));
            ...
            expect(mockShellProcess).toHaveBeenCalledWith(expect.stringContaining(SHELL_INJECTION_TRIGGER), expect.any(Object));
            // 2. Default processor runs second
            expect(defaultProcessMock).toHaveBeenCalledWith(expect.stringContaining('-shell-processed'), expect.any(Object));
            if (result?.type === 'submit_prompt') {
