...
import { ApprovalMode } from '@ouroboros/code-cli-core';
import os from 'os';
...
describe('ShellProcessor', () => {
    ...
    it('should process a single valid shell injection if allowed', async () => {
        ...
        expect(mockShellExecute).toHaveBeenCalledWith('git status', expect.any(String), expect.any(Function), expect.any(Object), false);
    ...
    it('should NOT throw ConfirmationRequiredError if a command is not allowed but approval mode is YOLO', async () => {
        ...
        expect(mockShellExecute).toHaveBeenCalledWith('rm -rf /', expect.any(String), expect.any(Function), expect.any(Object), false);
    ...
    it('should trim whitespace from the command inside the injection before interpolation', async () => {
        ...
        expect(mockShellExecute).toHaveBeenCalledWith(expectedCommand, expect.any(String), expect.any(Function), expect.any(Object), false);
    });
    ...
    describe('Robust Parsing (Balanced Braces)', () => {
        it('should correctly parse commands containing nested braces (e.g., awk)', async () => {
            ...
            const result = await processor.process(prompt, context);
            expect(mockCheckCommandPermissions).toHaveBeenCalledWith(command, expect.any(Object), context.session.sessionShellAllowlist);
            expect(mockShellExecute).toHaveBeenCalledWith(command, expect.any(String), expect.any(Function), expect.any(Object), false);
        ...
        it('should throw an error for unclosed shell injections', async () => {
            ...
            await expect(processor.process(prompt, context)).rejects.toThrow(/Unclosed shell injection/);
        });
    ...
    describe('Error Reporting', () => {
        it('should append exit code and command name on failure', async () => {
            ...
            const result = await processor.process(prompt, context);
            expect(result).toBe("some error output\n[Shell command 'cmd' exited with code 1]");
        });
