...
import { GeminiEventType } from '@ouroboros/ouroboros-code-core';
...
class CoderAgentExecutor implements AgentExecutor {
  ...
  async reconstruct(
    ...
    const agentsClient = runtimeTask.config.getConversationClient();
    if (!agentsClient.isInitialized()) {
      await agentsClient.initialize(
