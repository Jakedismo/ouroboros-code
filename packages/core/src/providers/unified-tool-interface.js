...
(function (ToolCategory) {
    ToolCategory["FILE_OPERATIONS"] = "file_operations";
    ToolCategory["FILE_DISCOVERY"] = "file_discovery";
    ToolCategory["SYSTEM_OPERATIONS"] = "system_operations";
    ToolCategory["WEB_OPERATIONS"] = "web_operations";
    ToolCategory["MEMORY_MANAGEMENT"] = "memory_management";
})(ToolCategory || (ToolCategory = {}));
/**
 * Security levels for tool operations
 */
export var ToolSecurityLevel;
(function (ToolSecurityLevel) {
    ToolSecurityLevel["SAFE"] = "safe";
    ToolSecurityLevel["MODERATE"] = "moderate";
    ToolSecurityLevel["DANGEROUS"] = "dangerous";
...
export class UnifiedToolInterface {
    ...
    static BUILTIN_TOOLS = {
        ...
        read_file: {
            ...
            description: 'Reads and returns the content of a specified file with support for pagination',
            capabilities: [
        ...
        read_many_files: {
            ...
            category: ToolCategory.FILE_OPERATIONS,
            securityLevel: ToolSecurityLevel.SAFE,
            description: 'Reads multiple files efficiently with batching and filtering',
            capabilities: [
        ...
        shell_command: {
            ...
            description: 'Executes system commands with security controls and monitoring',
            capabilities: [
    ...
    static getToolsByCategory(category) {
        return Object.values(this.BUILTIN_TOOLS).filter((tool) => tool.category === category);
    }
    /**
     * Get tools by security level
     */
    static getToolsBySecurityLevel(level) {
        return Object.values(this.BUILTIN_TOOLS).filter((tool) => tool.securityLevel === level);
    }
    /**
     * Get tools that require confirmation
     */
    static getToolsRequiringConfirmation() {
        return Object.values(this.BUILTIN_TOOLS).filter((tool) => tool.requiresConfirmation);
    }
    /**
     * Get tools that can modify files
     */
    static getFileModifyingTools() {
        return Object.values(this.BUILTIN_TOOLS).filter((tool) => tool.canModifyFiles);
    }
    /**
     * Get tools that can execute commands
     */
    static getCommandExecutingTools() {
        return Object.values(this.BUILTIN_TOOLS).filter((tool) => tool.canExecuteCommands);
    }
    /**
     * Get tools that can access network
     */
    static getNetworkAccessingTools() {
        return Object.values(this.BUILTIN_TOOLS).filter((tool) => tool.canAccessNetwork);
    }
    /**
     * Check if a tool is supported by a specific provider
     */
    static isToolSupportedByProvider(toolName, provider) {
    ...
    static getToolStatistics() {
        ...
        // Initialize counters
        Object.values(ToolCategory).forEach((category) => (byCategory[category] = 0));
        Object.values(ToolSecurityLevel).forEach((level) => (bySecurityLevel[level] = 0));
        tools.forEach((tool) => {
     ...
     */
    static metadataToUnifiedTool(metadata) {
        return {
            name: metadata.name,
        ...
        tools.forEach((tool) => {
            expectedProviders.forEach((provider) => {
                if (!tool.supportedProviders.includes(provider)) {
                    issues.push(`Tool ${tool.name} does not support provider ${provider}`);
                }
            });
        });
...
//# sourceMappingURL=unified-tool-interface.js.map