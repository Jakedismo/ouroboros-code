...
export class ToolBehaviorManager {
   ...
   * @private
   */
  private requiresFileConfirmation(): boolean {
   ...
   * @private
   */
  private shouldConfirmMemory(): boolean {
   ...
   * @private
   */
  private shouldConfirmWebFetch(): boolean {
   ...
   * @private
   */
  private getShellAllowlist(): Set<string> {
  ...
  validateToolSecurity(toolName: string, params: Record<string, any>): string | null {
    ...
    if (toolName === 'run_shell_command' && params['command'] && config.allowlist) {
      ...
      const isAllowed = Array.from(config.allowlist).some(allowedCmd => 
        command === allowedCmd || command.startsWith(`${allowedCmd} `)
      );
