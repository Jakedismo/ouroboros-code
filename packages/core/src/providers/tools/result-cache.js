...
export class ToolResultCache extends EventEmitter {
    cache = new Map();
    accessOrder = new Set(); // For LRU
    accessFrequency = new Map(); // For LFU
    config;
    stats;
    cleanupTimer = null;
    constructor(config = {}) {
     ...
     */
    set(toolCall, result, providerId, customTTL) {
     ...
     */
    invalidate(toolName, providerId) {
        let invalidatedCount = 0;
        const keysToDelete = [];
        for (const [key, entry] of this.cache.entries()) {
            if (entry.toolCall.name === toolName &&
     ...
     */
    createCacheKey(toolCall, providerId) {
        const argsHash = this.hashToolArguments(toolCall.arguments);
     ...
     */
    decompressResult(entry) {
        if (!entry.compressed) {
            return entry.result;
        }
        try {
            // Simple decompression simulation
            const { _compressed, ...result } = entry.result;
            return result;
        }
        catch (error) {
     ...
     */
    performEviction(aggressive = false, targetSpace = 0) {
        const maxSizeBytes = this.config.maxSizeMB * 1024 * 1024;
        const currentSize = this.getCurrentCacheSize();
     ...
     */
    selectEvictionCandidates(targetBytes) {
        const candidates = [];
     ...
     */
    selectLRUCandidates(targetBytes) {
        const candidates = [];
        let freedBytes = 0;
     ...
     * @returns Array of keys to evict.
     */
    selectLFUCandidates(targetBytes) {
     ...
     * @returns Array of keys to evict.
     */
    selectTTLCandidates(targetBytes) {
        const entries = Array.from(this.cache.entries());
     ...
     * @returns Array of keys to evict.
     */
    selectHybridCandidates(targetBytes) {
        const now = Date.now();
        const entries = Array.from(this.cache.entries());
     ...
     */
    calculateEvictionScore(key, entry, now) {
        const age = now - entry.createdAt.getTime();
 ...
 */
export function createResultCache(providerId, customConfig) {
