...
export class EnhancedWebFetch {
    ...
    async fetch(url, options = {}) {
        ...
        if (!securityInfo.allowed && !options.bypassSecurityChecks) {
            throw new FetchError(`URL blocked by security policy: ${securityInfo.reason}`, 'SECURITY_BLOCKED');
        }
        // Use security-recommended timeout
        const timeoutUsed = Math.min(options.timeout || securityInfo.recommendedTimeout, securityInfo.recommendedTimeout);
        // Sanitize URL
        ...
        if (options.context) {
            ...
            if (!contextValidation.valid && !options.bypassSecurityChecks) {
                throw new FetchError(`Request context validation failed: ${contextValidation.errors.join(', ')}`, 'CONTEXT_INVALID');
            }
        }
        ...
        try {
            ...
            if (response.redirected) {
                ...
                if (!finalUrlValidation.allowed && !options.bypassSecurityChecks) {
                    throw new FetchError(`Final URL after redirect blocked by security policy: ${finalUrlValidation.reason}`, 'REDIRECT_BLOCKED');
                }
            }
            ...
            if (contentLength && parseInt(contentLength) > maxSize) {
                throw new FetchError(`Response size (${contentLength} bytes) exceeds maximum allowed (${maxSize} bytes)`, 'SIZE_EXCEEDED');
            }
            ...
            if (isNodeError(error) && error.name === 'AbortError') {
                throw new FetchError(`Request timed out after ${timeoutUsed}ms`, 'ETIMEDOUT');
            }
            if (error instanceof FetchError) {
                throw error;
            }
            throw new FetchError(`Fetch failed: ${getErrorMessage(error)}`, 'FETCH_ERROR');
        }
        finally {
     ...
     * Validate response headers for security concerns
     */
    validateResponseHeaders(response, bypassChecks) {
        if (bypassChecks)
            return;
        ...
        for (const [name, value] of response.headers.entries()) {
            for (const pattern of maliciousPatterns) {
                if (pattern.test(`${name}: ${value}`)) {
                    throw new FetchError(`Malicious header pattern detected: ${name}: ${value}`, 'MALICIOUS_HEADER');
                }
            }
        }
    }
     ...
     */
    async readResponseSafely(response, maxLength) {
 ...
 */
export async function secureFetchText(url, options = {}) {
    return defaultEnhancedWebFetch.fetchText(url, options);
}
//# sourceMappingURL=enhanced-web-fetch.js.map