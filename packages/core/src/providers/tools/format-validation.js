...
export class ToolFormatValidator {
     ...
     */
    static validateUnifiedTool(tool, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!tool.name ||
            ...
            tool.name.trim() === '') {
            result.errors.push('Tool name is required and must be a non-empty string');
        ...
        if (!tool.parameters || typeof tool.parameters !== 'object') {
            ...
            const paramValidation = this.validateParameterSchema(tool.parameters, context, 'parameters');
     ...
     * @returns Validation result.
     */
    static validateParameterSchema(schema, context, path) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!schema.properties || typeof schema.properties !== 'object') {
            ...
            for (const [propName, propSchema] of Object.entries(schema.properties)) {
                const propValidation = this.validatePropertySchema(propSchema, context, `${path}.${propName}`);
        ...
        if (schema.required && !Array.isArray(schema.required)) {
            ...
            for (const requiredProp of schema.required) {
                if (typeof requiredProp !== 'string') {
                    ...
                    result.errors.push(`${path}: Required property '${requiredProp}' not found in properties`);
                    result.isValid = false;
                }
            }
        }
     ...
     * @returns Validation result.
     */
    static validatePropertySchema(schema, context, path) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!schema.type || !validTypes.includes(schema.type)) {
            result.errors.push(`${path}: Invalid or missing type. Must be one of: ${validTypes.join(', ')}`);
            result.isValid = false;
        }
        ...
        switch (schema.type) {
            case 'array':
                if (!schema.items) {
                    result.warnings.push(`${path}: Array type should have items definition`);
                }
                else {
                    const itemValidation = this.validatePropertySchema(schema.items, context, `${path}[items]`);
            ...
            case 'object':
                if (!schema.properties) {
                    result.warnings.push(`${path}: Object type should have properties definition`);
                }
                else {
                    for (const [propName, propSchema] of Object.entries(schema.properties)) {
                        const propValidation = this.validatePropertySchema(propSchema, context, `${path}.${propName}`);
            ...
            case 'number':
                if (schema.minimum !== undefined && schema.maximum !== undefined) {
                    if (schema.minimum > schema.maximum) {
                        result.errors.push(`${path}: Minimum cannot be greater than maximum`);
                        result.isValid = false;
                    }
                }
                break;
            case 'string':
                if (schema.minLength !== undefined && schema.maxLength !== undefined) {
                    if (schema.minLength > schema.maxLength) {
                        result.errors.push(`${path}: minLength cannot be greater than maxLength`);
                        result.isValid = false;
                    }
                }
                if (schema.pattern) {
     ...
     * @returns Validation result.
     */
    static validateForProvider(tool, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        switch (context.providerId) {
            ...
            default:
                result.warnings.push(`Unknown provider ${context.providerId}, skipping provider-specific validation`);
        }
        return result;
    }
    /**
     ...
     */
    static validateForOpenAI(tool, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!/^[a-zA-Z0-9_-]+$/.test(tool.name)) {
            result.errors.push('OpenAI function names must contain only letters, numbers, underscores, and hyphens');
            result.isValid = false;
        }
        // Description length check
        if (tool.description.length > 1024) {
            result.warnings.push('OpenAI function descriptions over 1024 characters may be truncated');
        }
        return result;
    }
    /**
     ...
     */
    static validateForAnthropic(tool, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(tool.name)) {
            result.errors.push('Anthropic tool names must start with a letter and contain only letters, numbers, and underscores');
            result.isValid = false;
        }
        return result;
    }
     ...
     */
    static validateForGemini(tool, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!/^[a-zA-Z0-9_.-]+$/.test(tool.name)) {
            result.errors.push('Gemini function names must contain only letters, numbers, underscores, dots, and hyphens');
            result.isValid = false;
        }
        return result;
    }
     ...
     * @returns Validation result.
     */
    static validateToolCall(toolCall, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!toolCall.arguments || typeof toolCall.arguments !== 'object') {
            result.errors.push('Tool call arguments are required and must be an object');
            result.isValid = false;
        }
     ...
     * @returns Validation result.
     */
    static validateToolResult(toolResult, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (!toolResult.toolCallId || typeof toolResult.toolCallId !== 'string') {
            result.errors.push('Tool result toolCallId is required and must be a string');
            result.isValid = false;
        }
        ...
        if (toolResult.isError && !toolResult.error) {
            result.warnings.push('Tool result marked as error but no error message provided');
        }
        if (toolResult.error && !toolResult.isError) {
            result.warnings.push('Tool result has error message but isError is false');
        }
        return result;
    }
    /**
     ...
     */
    static validateConversion(originalTool, converter, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        try {
            ...
            if (!providerFormat) {
                result.errors.push('Conversion to provider format returned null/undefined');
                result.isValid = false;
                return result;
            }
            ...
            if (converter.fromFunctionDeclaration) {
                try {
                    const functionDeclaration = converter.toFunctionDeclaration(originalTool);
                    const roundTripTool = converter.fromFunctionDeclaration(functionDeclaration);
                    // Check if essential fields are preserved
                    if (roundTripTool.name !== originalTool.name) {
                        result.warnings.push('Tool name changed during round-trip conversion');
                    }
                    if (roundTripTool.description !== originalTool.description) {
                        result.warnings.push('Tool description changed during round-trip conversion');
                    }
                }
                catch (conversionError) {
                    result.warnings.push(`Round-trip conversion failed: ${conversionError.message}`);
                }
            }
        }
        catch (error) {
     ...
     */
    static validateComplete(tool, converter, context) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        if (toolValidation.isValid) {
            const conversionValidation = this.validateConversion(tool, converter, context);
     ...
     */
    static createContext(providerId, options = {}) {
        return {
            providerId,
     ...
     */
    static toErrorIfInvalid(validation, context) {
        if (validation.isValid) {
            return null;
        }
        const errorMessage = validation.errors.join('; ');
        return ToolConversionError.schemaConversion('unified', context.providerId, errorMessage);
    }
}
//# sourceMappingURL=format-validation.js.map