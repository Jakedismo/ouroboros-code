...
export class ToolDiscoverySync extends EventEmitter {
     ...
     */
    registerProvider(providerId, toolManager) {
     ...
     */
    async synchronizeTools(forceSync = false) {
        ...
        try {
            ...
            for (const [providerId, toolManager] of this.providers.entries()) {
                ...
                const discoveryPromise = this.discoverProviderTools(providerId, toolManager, state).catch((error) => {
     ...
     */
    getSynchronizedTools(includingConflicted = false) {
        const tools = [];
        for (const syncedTool of this.synchronizedTools.values()) {
            if (includingConflicted ||
     ...
     */
    async resolveConflict(toolName, selectedVersion) {
        const conflict = this.toolConflicts.get(toolName);
        if (!conflict) {
            return false;
        }
        const [providerId, version] = selectedVersion.split(':');
        const selectedTool = conflict.conflictingVersions.find((tool) => tool.providerId === providerId && tool.version.toString() === version);
        if (!selectedTool) {
            return false;
        }
     ...
     */
    async discoverProviderTools(providerId, toolManager, state) {
        ...
        try {
            ...
            for (const unifiedTool of unifiedTools) {
                ...
                const syncedTool = {
                    tool: unifiedTool,
                    originalTool: toolManager.getTool(unifiedTool.name),
                    providerId,
    ...
    async applyConflictResolution(conflict) {
        ...
        switch (resolutionStrategy) {
            case 'latest':
                ...
                return conflictingVersions.reduce((latest, current) => current.discoveryTime > latest.discoveryTime ? current : latest);
            case 'provider-priority':
                // Use provider priority order
                for (const priority of this.config.providerPriority) {
                    const tool = conflictingVersions.find((t) => t.providerId === priority);
                    if (tool)
                        return tool;
                }
     ...
     * @returns Server name.
     */
    extractServerName(toolName, toolManager) {
     ...
     */
    cacheDiscovery(providerId, tools) {
     ...
     * @param state Provider discovery state.
     */
    processCachedTools(providerId, cachedTools, state) {
    ...
    updateStats() {
        ...
        this.stats.activeProviders = Array.from(this.providerStates.values()).filter((state) => state.lastSuccessfulSync).length;
        this.stats.totalTools = this.synchronizedTools.size;
        this.stats.uniqueTools = new Set(Array.from(this.synchronizedTools.keys())).size;
        this.stats.conflictCount = this.toolConflicts.size;
        // Update provider stats
        this.stats.providerStats = Object.fromEntries(this.providerStates.entries());
    }
    /**
 ...
 */
export function createToolDiscoverySync(customConfig) {
    return new ToolDiscoverySync(customConfig);
}
//# sourceMappingURL=tool-discovery-sync.js.map