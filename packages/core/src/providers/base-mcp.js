 ...
 */
export class BaseMCPProvider extends BaseLLMProvider {
    mcpToolManager;
    mcpConfig;
    constructor(config) {
        super(config);
        // Merge MCP configuration with defaults
        this.mcpConfig = MultiProviderMCPConfigMerger.merge(config.mcpConfig || {}, DEFAULT_MULTI_PROVIDER_MCP_CONFIG);
    ...
    async executeToolsWithMCP(calls) {
        ...
        if (parallelExecution && calls.length > 1) {
            ...
            for (let i = 0; i < calls.length; i += maxConcurrent) {
                ...
                const batchPromises = batch.map(call => toolManager.executeTool(call).catch(error => ({
                    toolCallId: call.id,
     ...
     */
    async syncToolsAcrossProviders() {
        if (!this.isMCPEnabled()) {
            return;
        }
        const toolManager = this.getMCPToolManager();
     ...
     */
    isMCPEnabled() {
        return (this.config.enableMCP === true &&
    ...
    updateMCPConfig(updates) {
        this.mcpConfig = MultiProviderMCPConfigMerger.merge(updates, this.mcpConfig);
    }
    /**
...
//# sourceMappingURL=base-mcp.js.map