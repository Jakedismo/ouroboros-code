...
class ModelRegistry {
  ...
  getModelInfo(provider: LLMProvider, modelId: string): ModelInfo | undefined {
  ...
  compareModels(model1: ModelInfo, model2: ModelInfo): number {
    // Compare by context window size
    return (
      model2.capabilities.contextWindow - model1.capabilities.contextWindow
    );
  }
}
...
describe('ModelRegistry', () => {
  ...
  describe('Model Discovery', () => {
    it('should return available models for each provider', async () => {
      const geminiModels = await registry.getAvailableModels(
        LLMProvider.GEMINI,
      );
      ...
      const openaiModels = await registry.getAvailableModels(
        LLMProvider.OPENAI,
      );
      ...
      const anthropicModels = await registry.getAvailableModels(
    ...
    it('should check model availability correctly', async () => {
      expect(
        await registry.isModelAvailable(LLMProvider.GEMINI, 'gemini-2.5-pro'),
      ).toBe(true);
      expect(
        await registry.isModelAvailable(LLMProvider.GEMINI, 'gemini-1.5-pro'),
      ).toBe(false);

      expect(await registry.isModelAvailable(LLMProvider.OPENAI, 'gpt-5')).toBe(
        true,
      );
      expect(await registry.isModelAvailable(LLMProvider.OPENAI, 'gpt-4')).toBe(
        false,
      );

      expect(
        await registry.isModelAvailable(
          LLMProvider.ANTHROPIC,
          'claude-4-1-opus-20250805',
        ),
      ).toBe(true);
      expect(
        await registry.isModelAvailable(LLMProvider.ANTHROPIC, 'claude-3-opus'),
      ).toBe(false);
    });

    it('should get model info correctly', () => {
      const geminiPro = registry.getModelInfo(
        LLMProvider.GEMINI,
        'gemini-2.5-pro',
      );
  ...
  describe('Model Selection', () => {
    it('should set and get current model', async () => {
      ...
      expect(registry.getCurrentModel(LLMProvider.GEMINI)).toBe(
        'gemini-2.5-flash',
      );
    ...
    it('should return default model when not set', () => {
      ...
      expect(registry.getCurrentModel(LLMProvider.ANTHROPIC)).toBe(
    ...
    it('should throw error when setting unavailable model', async () => {
      await expect(
        registry.setModel(LLMProvider.GEMINI, 'non-existent-model'),
      ).rejects.toThrow('Model non-existent-model not available for gemini');
    });
  });
  ...
  describe('Model Equivalencies', () => {
    it('should find equivalent models for special keywords', () => {
      ...
      expect(bestAvailable.get(LLMProvider.ANTHROPIC)).toBe(
        'claude-4-1-opus-20250805',
      );
      ...
      expect(fastest.get(LLMProvider.ANTHROPIC)).toBe(
        'claude-4-sonnet-20250514',
      );
    });
    ...
    it('should default to balanced models for unknown specs', () => {
      ...
      expect(unknown.get(LLMProvider.ANTHROPIC)).toBe(
        'claude-4-sonnet-20250514',
      );
    });
  });
  ...
  describe('Model Validation', () => {
    it('should validate model access based on API key', () => {
      ...
      expect(
        registry.validateModelAccess(LLMProvider.OPENAI, 'gpt-5', 'test-key'),
      ).toBe(true);

      // Premium models require premium key
      expect(
        registry.validateModelAccess(LLMProvider.OPENAI, 'o3', 'regular-key'),
      ).toBe(false);
      expect(
        registry.validateModelAccess(LLMProvider.OPENAI, 'o3', 'premium-key'),
      ).toBe(true);
      expect(
        registry.validateModelAccess(
          LLMProvider.ANTHROPIC,
      ...
      expect(
        registry.validateModelAccess(LLMProvider.GEMINI, 'gemini-2.5-pro'),
      ).toBe(false);
    });
  });
  ...
  describe('Model Comparison', () => {
    it('should compare models by context window', () => {
      ...
      const geminiFlash = registry.getModelInfo(
        LLMProvider.GEMINI,
        'gemini-2.5-flash',
      )!;
      const o3 = registry.getModelInfo(LLMProvider.OPENAI, 'o3')!;
  ...
  describe('Pricing Information', () => {
    it('should provide pricing information', () => {
      ...
      const claudeOpus = registry.getModelInfo(
        LLMProvider.ANTHROPIC,
        'claude-4-1-opus-20250805',
      )!;
