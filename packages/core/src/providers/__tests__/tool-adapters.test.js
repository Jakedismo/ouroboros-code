...
describe('Tool Adapters', () => {
    ...
    describe('OpenAIToolAdapter', () => {
        ...
        describe('toProviderFormat', () => {
            ...
            it('should handle complex nested schemas', () => {
                ...
                expect(result.parameters.properties.config.properties.options.items
                    .properties.metadata).toBeDefined();
                expect(result.parameters.properties.config.properties.options.items
                    .properties.metadata.properties.tags.items.type).toBe('string');
            });
        ...
        describe('streaming tool calls', () => {
            it('should accumulate streaming tool calls correctly', () => {
                ...
                expect(accumulated[0].function.arguments).toBe('{"query":"TypeScript"}');
            });
            it('should handle multiple concurrent tool calls', () => {
                ...
                const accumulated = adapter.accumulateStreamingToolCall(multiToolChunk, []);
        ...
        describe('formatToolsForRequest', () => {
            ...
            it('should handle single required tool', () => {
                const result = adapter.formatToolsForRequest([sampleUnifiedTool], 'search_web');
                expect(result.tool_choice).toBe('auto');
            });
        });
    });
    describe('AnthropicToolAdapter', () => {
        ...
        describe('toProviderFormat', () => {
            ...
            it('should handle complex nested schemas', () => {
                ...
                const result = adapter.toProviderFormat(complexTool);
                expect(result.input_schema.properties.data.items.properties.metrics
                    .properties.score.minimum).toBe(0);
                expect(result.input_schema.properties.data.items.properties.metrics
                    .properties.score.maximum).toBe(100);
            });
        });
        describe('fromProviderToolCall', () => {
        ...
        describe('toProviderToolResult', () => {
            ...
            it('should handle complex object content', () => {
                ...
                expect(result.content).toBe(JSON.stringify(objectResult.content, null, 2));
            });
        });
        describe('streaming tool use', () => {
            it('should accumulate streaming tool use correctly', () => {
                ...
                let currentToolUse = adapter.accumulateStreamingToolUse(startEvent, null);
                expect(currentToolUse?.id).toBe('toolu_stream1');
                expect(currentToolUse?.name).toBe('search_web');
                currentToolUse = adapter.accumulateStreamingToolUse(deltaEvent, currentToolUse);
                currentToolUse = adapter.accumulateStreamingToolUse(deltaEvent2, currentToolUse);
    ...
    describe('Cross-Adapter Compatibility', () => {
        ...
        it('should produce consistent tool names across adapters', () => {
            ...
            const anthropicFormat = anthropicAdapter.toProviderFormat(sampleUnifiedTool);
        ...
        it('should handle identical tool calls with different IDs', () => {
            ...
            const anthropicResult = anthropicAdapter.fromProviderToolCall(anthropicCall);
        ...
        it('should maintain parameter schema integrity across conversions', () => {
            ...
            expect(openaiSchema.properties.filters.properties.categories.items.enum).toEqual(['news', 'research', 'blog']);
            expect(anthropicSchema.properties.filters.properties.categories.items.enum).toEqual(['news', 'research', 'blog']);
        });
    });
    ...
    describe('Edge Cases', () => {
        it('should handle tools with no parameters', () => {
            ...
            expect(() => anthropicAdapter.toProviderFormat(noParamTool)).not.toThrow();
        });
        it('should handle very large parameter schemas', () => {
            const largeTool = {
                ...
                parameters: {
                    type: 'object',
                    properties: Object.fromEntries(Array.from({ length: 50 }, (_, i) => [
                        `param_${i}`,
                        {
                            type: 'string',
        ...
        it('should handle null and undefined values in tool results', () => {
            ...
            expect(() => openaiAdapter.toProviderToolResult(nullResult)).not.toThrow();
            expect(() => openaiAdapter.toProviderToolResult(undefinedResult)).not.toThrow();
            expect(() => anthropicAdapter.toProviderToolResult(nullResult)).not.toThrow();
            expect(() => anthropicAdapter.toProviderToolResult(undefinedResult)).not.toThrow();
        });
    ...
    describe('Integration with Real-World Schemas', () => {
        it('should handle OpenAPI-style schemas', () => {
            ...
            expect(openaiResult.parameters.properties.url.pattern).toBe('^https?://.*');
            expect(anthropicResult.input_schema.properties.url.pattern).toBe('^https?://.*');
        });
    });
...
//# sourceMappingURL=tool-adapters.test.js.map