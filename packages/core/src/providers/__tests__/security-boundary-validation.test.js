    ...
    describe('File System Security Boundaries', () => {
        describe('Path Traversal Prevention', () => {
            ...
            it('should validate security using BuiltinToolManagerHelpers', async () => {
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(dangerousToolCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
        ...
        describe('File Operation Security', () => {
            it('should validate batch file operations securely', async () => {
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(batchToolCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
                expect(validation.allowed).toBe(false);
    ...
    describe('Shell Command Security Boundaries', () => {
        ...
        describe('Safe Command Validation', () => {
            ...
            it('should validate shell security using BuiltinToolManagerHelpers', async () => {
                const dangerousShellCall = {
                    id: 'security-test-shell',
                    name: 'run_shell_command',
                    parameters: { command: 'rm -rf /' },
                };
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(dangerousShellCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
                expect(validation.allowed).toBe(false);
    ...
    describe('Web Request Security Boundaries', () => {
        ...
        describe('Content and Rate Limiting', () => {
            ...
            it('should validate web requests using BuiltinToolManagerHelpers', async () => {
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(maliciousWebCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
                expect(validation.allowed).toBe(false);
                expect(validation.riskLevel).toBe('HIGH');
                expect(validation.reason).toContain('Blocked URLs detected');
            });
            it('should detect suspicious search queries', async () => {
                const suspiciousSearchCall = {
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(suspiciousSearchCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
                expect(validation.allowed).toBe(false);
    ...
    describe('Memory and Resource Security', () => {
        describe('Memory Safety', () => {
            it('should validate memory operations securely', async () => {
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(memoryToolCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'MODERATE' }) }, fileSystemBoundary, shellSecurity, webHandler);
            ...
            it('should prevent excessive memory allocation attempts', async () => {
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(excessiveMemoryCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
    ...
    describe('Cross-Provider Security Consistency', () => {
        describe('Tool Availability Security', () => {
            it('should respect tool enablement configuration', async () => {
                const restrictiveToolBehaviors = {
                    isToolAllowed: (toolName) => toolName === 'read_file', // Only allow read_file
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(disallowedToolCall, mockContext, restrictiveToolBehaviors, fileSystemBoundary, shellSecurity, webHandler);
                expect(validation.allowed).toBe(false);
            ...
            it('should handle security validation errors gracefully', async () => {
                ...
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(toolCall, mockContext, faultyToolBehaviors, fileSystemBoundary, shellSecurity, webHandler);
        ...
        describe('Confirmation Flow Security', () => {
            ...
            it('should require confirmation for high-risk operations', async () => {
                const highRiskToolCall = {
                    id: 'high-risk-test',
                    name: 'run_shell_command',
                    parameters: { command: 'npm install malicious-package' },
                };
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(highRiskToolCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
                if (validation.allowed) {
                    expect(validation.requiresConfirmation).toBe(true);
                    expect(validation.riskLevel).toBeOneOf(['HIGH', 'CRITICAL']);
                }
    ...
    describe('End-to-End Security Integration', () => {
        it('should validate complex multi-tool security scenarios', async () => {
            ...
            for (const toolCall of complexScenario) {
                const validation = await BuiltinToolManagerHelpers.validateToolSecurity(toolCall, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler);
                expect(validation.allowed).toBe(false);
                expect(validation.riskLevel).toBeOneOf(['HIGH', 'CRITICAL']);
        ...
        it('should maintain security boundaries under load', async () => {
            ...
            const concurrentValidations = Array.from({ length: 50 }, (_, i) => BuiltinToolManagerHelpers.validateToolSecurity({
                id: `load-test-${i}`,
                name: 'read_file',
                parameters: { file_path: '/etc/passwd' },
            }, mockContext, { isToolAllowed: () => true, getToolConfig: () => ({ securityLevel: 'HIGH' }) }, fileSystemBoundary, shellSecurity, webHandler));
...
//# sourceMappingURL=security-boundary-validation.test.js.map