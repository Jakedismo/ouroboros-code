...
describe('Provider Tool Integration', () => {
    ...
    beforeEach(() => {
        ...
        toolRegistry = {
            ...
            getToolByName: vi.fn().mockImplementation((name) => mockTools.get(name)),
        };
        ...
        toolManager = {
            ...
            getToolCategory: vi.fn().mockImplementation((toolName) => {
        ...
        BuiltinToolManager.mockImplementation(() => toolManager);
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    ...
    describe('OpenAI Provider Integration', () => {
        ...
        it('should execute file system tools through OpenAI provider', async () => {
            ...
            const result = await openaiProvider.generateContent(messages, { maxTokens: 1000 }, mockAbortController.signal);
            expect(result).toBeDefined();
            expect(global.fetch).toHaveBeenCalled();
            // Verify tool was executed through the tool manager
            expect(toolManager.executeTool).toHaveBeenCalledWith(expect.objectContaining({
                name: 'read_file',
        ...
        it('should handle OpenAI multi-turn conversation with tools', async () => {
            ...
            const result = await openaiProvider.generateContent(conversation, { maxTokens: 1000 }, mockAbortController.signal);
            expect(result).toBeDefined();
            expect(global.fetch).toHaveBeenCalled();
        });
        it('should handle OpenAI error scenarios gracefully', async () => {
            ...
            await expect(openaiProvider.generateContent(messages, {}, mockAbortController.signal)).rejects.toThrow();
        });
    });
    describe('Anthropic Provider Integration', () => {
        ...
        it('should execute shell tools through Anthropic provider', async () => {
            ...
            const result = await anthropicProvider.generateContent(messages, { maxTokens: 1000 }, mockAbortController.signal);
            expect(result).toBeDefined();
            expect(global.fetch).toHaveBeenCalled();
            // Verify tool was executed through the tool manager
            expect(toolManager.executeTool).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should handle Anthropic multi-turn conversation with tools', async () => {
            const conversation = [
                ...
                { role: 'user', content: 'Now create a summary file' },
            ];
            const result = await anthropicProvider.generateContent(conversation, { maxTokens: 1000 }, mockAbortController.signal);
            expect(result).toBeDefined();
            expect(global.fetch).toHaveBeenCalled();
        });
        it('should handle Anthropic error scenarios gracefully', async () => {
            ...
            const messages = [{ role: 'user', content: 'Test message' }];
            await expect(anthropicProvider.generateContent(messages, {}, mockAbortController.signal)).rejects.toThrow();
        });
    });
    ...
    describe('Performance and Reliability', () => {
        ...
        it('should handle concurrent tool execution across providers', async () => {
            ...
            const openaiPromises = toolCalls.slice(0, 5).map(call => toolManager.executeTool(call, {
                provider: 'openai',
                conversationId: 'test',
                userId: 'user',
                signal: mockAbortController.signal,
            ...
            const anthropicPromises = toolCalls.slice(5).map(call => toolManager.executeTool(call, {
                provider: 'anthropic',
                conversationId: 'test',
                userId: 'user',
                signal: mockAbortController.signal,
        ...
        it('should handle timeout scenarios consistently', async () => {
            ...
            toolManager.executeTool = vi.fn().mockImplementation(() => new Promise((resolve, reject) => {
            ...
            await expect(toolManager.executeTool(toolCall, {
                provider: 'openai',
                conversationId: 'test',
                userId: 'user',
...
//# sourceMappingURL=provider-tool-integration.test.js.map