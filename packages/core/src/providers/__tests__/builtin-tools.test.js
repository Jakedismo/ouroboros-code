...
describe('BuiltinToolManager', () => {
    ...
    describe('tool categorization', () => {
        it('should correctly categorize file system tools', async () => {
            const tools = await toolManager.getAvailableTools();
            const fileSystemTools = tools.filter(t => toolManager.getToolCategory(t.name) === 'filesystem');
        ...
        it('should correctly categorize web tools', async () => {
            const tools = await toolManager.getAvailableTools();
            const webTools = tools.filter(t => toolManager.getToolCategory(t.name) === 'web');
        ...
        it('should correctly categorize system tools', async () => {
            const tools = await toolManager.getAvailableTools();
            const systemTools = tools.filter(t => toolManager.getToolCategory(t.name) === 'system');
    ...
    describe('security validation', () => {
        let mockToolBehaviors;
        let mockFileSystemBoundary;
        let mockShellSecurity;
        beforeEach(() => {
            mockToolBehaviors = {
            ...
            ToolBehaviorManager.mockImplementation(() => mockToolBehaviors);
            FileSystemBoundary.mockImplementation(() => mockFileSystemBoundary);
            ShellToolSecurity.mockImplementation(() => mockShellSecurity);
            toolManager = new BuiltinToolManager(config, toolRegistry);
        });
    ...
    describe('performance and caching', () => {
        ...
        it('should handle concurrent tool execution', async () => {
            ...
            const results = await Promise.all(toolCalls.map(toolCall => toolManager.executeTool(toolCall, mockContext)));
    ...
    describe('error handling and resilience', () => {
        it('should handle aborted tool execution', async () => {
            ...
            const mockTool = {
                build: vi.fn().mockReturnValue({
                    execute: vi.fn().mockImplementation(async (signal) => new Promise((resolve, reject) => {
