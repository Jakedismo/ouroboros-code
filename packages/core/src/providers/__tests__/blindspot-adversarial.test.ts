...
class MockOrchestrator {
  async executeParallel(request: unknown, providers?: LLMProvider[]) {
  ...
  async queryProvider(provider: LLMProvider, request: unknown) {
  ...
  private getMockResponse(provider: LLMProvider, prompt: string): string {
    const responses = {
  ...
  private getMockAnalysis(provider: LLMProvider, prompt: string): string {
...
describe('Blindspot Detection', () => {
  ...
  describe('BlindspotDetector', () => {
    ...
    it('should identify common blindspots', async () => {
      ...
      const hasSecurityBlindspot = analysis.commonBlindspots.some((b) =>
    ...
    it('should extract unique insights by provider', async () => {
      const analysis = await detector.detectBlindspots(
  ...
  describe('Blindspot Formatting', () => {
    it('should format analysis for display', async () => {
      const { formatBlindspotAnalysis } = await import(
        '../blindspot-detector.js'
      );
...
describe('Adversarial Challenge', () => {
  let challenger: AdversarialChallenger;
  let orchestrator: MockOrchestrator;
  ...
  describe('AdversarialChallenger', () => {
    ...
    it('should apply focus to challenges', async () => {
      ...
      const hasLogicalCritique = report.challenges.some((c) =>
        c.critique.toLowerCase().includes('logical'),
      );
  ...
  describe('Challenge Formatting', () => {
    it('should format challenge report for display', async () => {
      const { formatChallengeReport } = await import(
        '../adversarial-challenger.js'
      );
...
describe('Integration Tests', () => {
  it('should handle errors gracefully in blindspot detection', async () => {
    ...
    await expect(detector.detectBlindspots('Test')).rejects.toThrow(
      'API error',
    );
  });
  ...
  it('should handle errors gracefully in challenge', async () => {
    ...
    await expect(challenger.runChallenge('Test')).rejects.toThrow(
      'Provider failed',
    );
  });
