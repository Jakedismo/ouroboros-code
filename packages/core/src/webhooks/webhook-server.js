...
export class WebhookServer extends EventEmitter {
    ...
    validateHMAC(body, signature) {
        if (!signature || !this.config.hmacSecret)
            return false;
    ...
    registerCallback(toolId, callback) {
        this.activeCallbacks.set(toolId, callback);
    }
    unregisterCallback(toolId) {
...
let webhookServerInstance;
export function getWebhookServer(config) {
