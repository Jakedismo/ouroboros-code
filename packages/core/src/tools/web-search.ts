...
> {
  ...
  override async execute(signal: AbortSignal): Promise<WebSearchToolResult> {
    const contentGenerator = await this.config.getContentGenerator();

    try {
      const response = await contentGenerator.generateContent(
        { 
          contents: [{ role: 'user', parts: [{ text: this.params.query }] }], 
