...
// Mock Config constructor
vi.mock('../config/config.js');

describe('WebSearchTool', () => {
  const abortSignal = new AbortController().signal;
  let mockAgentsClient: { generateContent: Mock };
  let tool: WebSearchTool;

  beforeEach(() => {
    mockAgentsClient = { generateContent: vi.fn() as unknown as Mock } as unknown as { generateContent: Mock };
  ...
  describe('execute', () => {
    it('should return search results for a successful query', async () => {
      ...
      mockAgentsClient.generateContent.mockResolvedValue({
        candidates: [
          {
            content: {
              role: 'model',
    ...
    it('should return a WEB_SEARCH_FAILED error on failure', async () => {
      ...
      mockAgentsClient.generateContent.mockRejectedValue(testError);
        ...
        candidates: [
          {
            content: {
              role: 'model',
              parts: [{ text: 'こんにちは! Ouroboros Code✨️' }],
            },
            groundingMetadata: {
              groundingChunks: [
                {
                ...
                {
                  web: {
                    title: 'Ouroboros Code: your open-source AI agent',
              ...
              groundingSupports: [
                ...
                {
                  segment: {
                    // Byte range of "Ouroboros Code✨️" (utf-8 encoded)
                    startIndex: 17,
                    endIndex: 37,
                  },
...
[3] Ouroboros Code: your open-source AI agent (https://blog.google/technology/developers/introducing-gemini-cli-open-source-ai-agent/)`;
