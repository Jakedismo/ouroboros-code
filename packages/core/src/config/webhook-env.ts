...
// WEBHOOK_EXTENSION: Environment variable support for webhook configuration
import { WebhookConfiguration } from './webhook-config.js';

/**
 * Read webhook configuration from environment variables
 */
export function getWebhookConfigFromEnv(): Partial<WebhookConfiguration> {
  const config: Partial<WebhookConfiguration> = {};
  ...
  if (process.env['GEMINI_WEBHOOK_AUTH_MODE']) {
    ...
    if (authMode === 'token' || authMode === 'hmac' || authMode === 'both') {
      config.authMode = authMode as 'token' | 'hmac' | 'both';
    }
  }
