...
describe('Provider Switching and Compatibility Tests', () => {
    ...
    describe('Dynamic Provider Switching', () => {
        it('should switch providers within the same session', async () => {
            ...
            const geminiResult = await rig.run('Read switch-test.txt and identify yourself as the current provider.');
            expect(geminiResult).toBeTruthy();
            // Verify response content
            expect(geminiResult).toContain('switch-test.txt');
            // Test with OpenAI (if available)
            if (process.env.OPENAI_API_KEY) {
                const openaiResult = await rig.run('Read switch-test.txt again and identify yourself as the current provider.', '--provider', 'openai');
        ...
        it('should maintain tool execution consistency across provider switches', async () => {
            ...
            if (providers.length < 2) {
                console.warn('Skipping multi-provider test - need at least 2 providers configured');
                return;
            }
            ...
            for (const provider of providers) {
                ...
                const result = await rig.run('Read input.txt, create a processed version called output.txt with uppercase content, then list all files.', ...args);
            ...
            console.log('Provider comparison results:', results.map((r) => ({
                provider: r.provider,
    ...
    describe('Configuration Compatibility', () => {
        it('should handle provider-specific configuration overrides', async () => {
            ...
            for (const provider of providers) {
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                const result = await rig.run('Create a test file with some content using write_file tool.', ...args);
                expect(result).toBeTruthy();
                const toolLogs = rig.readToolLogs();
        ...
        it('should validate provider-specific model availability', async () => {
            ...
            for (const config of validConfigs) {
                ...
                const args = config.provider === 'gemini'
                    ? ['--model', config.model]
                ...
                try {
                    ...
                    console.warn(`Model ${config.model} for ${config.provider} may not be available:`, error.message);
                }
            }
        });
    });
    describe('Tool Format Conversion', () => {
        it('should convert tool parameters correctly across providers', async () => {
            ...
            for (const provider of providers) {
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                const result = await rig.run(`Create a JSON file called data.json with this content: ${complexContent}`, ...args);
                expect(result).toBeTruthy();
                const toolLogs = rig.readToolLogs();
                const writeFileCall = toolLogs.find((log) => log.toolRequest.name === 'write_file');
        ...
        it('should handle tool response formatting consistently', async () => {
            ...
            for (const provider of providers) {
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                const result = await rig.run('Read all three files (text.txt, data.json, script.js) and describe their contents.', ...args);
                expect(result).toBeTruthy();
                const toolLogs = rig.readToolLogs();
                const readFileCalls = toolLogs.filter((log) => log.toolRequest.name === 'read_file');
                ...
                responseFormats.push({
                    ...
                    mentionsJavaScript: result.toLowerCase().includes('javascript') ||
    ...
    describe('Error Handling and Fallbacks', () => {
        it('should handle provider failures gracefully', async () => {
            ...
            if (!process.env.OPENAI_API_KEY) {
                console.warn('Skipping provider failure test - no OpenAI key to invalidate');
                return;
            }
            // Try to use OpenAI with invalid key
            try {
                const result = await rig.run('Simple test that should fail with invalid API key', '--provider', 'openai');
        ...
        it('should validate tool compatibility warnings', async () => {
            ...
            const result = await rig.run('Use web_fetch to get content from httpbin.org and shell_command to echo the result.');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
            // Should attempt both tools
            const webFetchCalled = toolLogs.some((log) => log.toolRequest.name === 'web_fetch');
            const shellCommandCalled = toolLogs.some((log) => log.toolRequest.name === 'shell_command');
    ...
    describe('Performance Comparison', () => {
        it('should measure response times across providers', async () => {
            ...
            if (process.env.ANTHROPIC_API_KEY)
                providers.push('anthropic');
            if (providers.length < 2) {
                console.warn('Skipping performance comparison - need multiple providers');
                return;
            }
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                const result = await rig.run('Read perf-data.txt, create a summary, and list the directory contents.', ...args);
                ...
                performanceMetrics.push({
                    ...
                    averageToolTime: toolLogs.length > 0
        ...
        it('should handle concurrent requests across providers', async () => {
            ...
            for (let i = 1; i <= 3; i++) {
                rig.createFile(`concurrent-${i}.txt`, `Concurrent test file ${i} content`);
            }
            const promises = [];
            const providers = ['gemini'];
            if (process.env.OPENAI_API_KEY)
            ...
            for (let i = 0; i < providers.length; i++) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                const promise = rig
                    .run(`Read concurrent-${(i % 3) + 1}.txt and summarize its content.`, ...args)
                    .then((result) => ({
                    provider,
                    result,
                    success: !!result,
            ...
            console.log('Concurrent execution results:', results.map((r) => ({
                provider: r.provider,
                length: r.result.length,
                success: r.success,
            })));
    ...
    describe('Backward Compatibility', () => {
        it('should maintain compatibility with existing Gemini configurations', async () => {
            ...
            const result = await rig.run('Test backward compatibility by creating a test file with write_file tool.');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
            expect(toolLogs.some((log) => log.toolRequest.name === 'write_file')).toBe(true);
        });
        it('should handle legacy command-line arguments', async () => {
            ...
            const result = await rig.run('Create a test file using write_file tool.', '--yolo');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
            expect(toolLogs.some((log) => log.toolRequest.name === 'write_file')).toBe(true);
        });
        it('should migrate settings format automatically', async () => {
            ...
            const result = await rig.run('Test settings migration by using read_file tool.');
            expect(result).toBeTruthy();
...
//# sourceMappingURL=provider-switching.test.js.map