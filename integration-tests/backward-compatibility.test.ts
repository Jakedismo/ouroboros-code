...
describe('Backward Compatibility Tests', () => {
  ...
  describe('Default Provider Behavior', () => {
    ...
    it('should maintain existing tool execution patterns', async () => {
      ...
      for (const pattern of patterns) {
        ...
        const toolCalled = toolLogs.some(
          (log) => log.toolRequest.name === pattern.expectedTool,
        );

        if (!toolCalled) {
          throw new Error(
            createToolCallErrorMessage(
              pattern.expectedTool,
              toolLogs.map((t) => t.toolRequest.name),
              result,
            ),
          );
        }
    ...
    it('should handle the same error scenarios as before', async () => {
      ...
      const result = await rig.run(
        'Try to read a non-existent file called missing.txt',
      );
  ...
  describe('Legacy Configuration Support', () => {
    it('should support legacy Gemini-only settings format', async () => {
      ...
      const result = await rig.run(
        'Create a test file using write_file tool to verify legacy config works.',
      );
    ...
    it('should handle mixed legacy and new configuration formats', async () => {
      ...
      const result = await rig.run(
        'Test mixed configuration by reading a file and creating output.',
      );
    ...
    it('should migrate settings automatically when possible', async () => {
      ...
      const result = await rig.run(
        'Test settings migration by using tools to verify configuration works.',
      );
  ...
  describe('Legacy Command-Line Arguments', () => {
    it('should support existing command-line flags', async () => {
      ...
      for (const flags of legacyFlags) {
        try {
          const result = await rig.run(
            'Read cli-test.txt and confirm the flag works.',
            ...flags,
          );
          ...
          const toolLogs = rig.readToolLogs();
          expect(
            toolLogs.some((log) => log.toolRequest.name === 'read_file'),
          ).toBe(true);
        } catch (error) {
          console.warn(
            `Legacy flag ${flags.join(' ')} may not be supported:`,
            error.message,
          );
        }
      }
    ...
    it('should handle legacy approval mode arguments', async () => {
      ...
      for (const mode of approvalModes) {
        try {
          const result = await rig.run(
            'Create a potentially dangerous file called test-dangerous.txt with content.',
            mode.flag,
          );
          ...
          console.warn(
            `Approval mode ${mode.flag} may have changed behavior:`,
            error.message,
          );
        }
      }
    ...
    it('should support legacy environment variable names', async () => {
      ...
      try {
        ...
        if (originalGeminiKey || originalGoogleKey) {
          const result = await rig.run(
            'Test environment variable compatibility by creating a test file.',
          );
  ...
  describe('Tool Compatibility and Behavior', () => {
    it('should maintain identical tool signatures and behavior', async () => {
      ...
      const toolTests = [
        {
          ...
          verify: (result: string) =>
            result.toLowerCase().includes('content to read'),
        },
        {
          tool: 'write_file',
          setup: () => {},
      ...
      for (const test of toolTests) {
        ...
        const toolLogs = rig.readToolLogs();
        expect(toolLogs.some((log) => log.toolRequest.name === test.tool)).toBe(
          true,
        );
    ...
    it('should maintain tool confirmation behavior', async () => {
      ...
      const result = await rig.run(
        'Create a system file that might require confirmation.',
        '--yolo', // Override to ensure test works
      );
    ...
    it('should maintain tool error reporting format', async () => {
      ...
      const result = await rig.run(
        'Try to use an invalid tool parameter to test error handling.',
      );
  ...
  describe('Output Format Compatibility', () => {
    it('should maintain consistent output formatting', async () => {
      ...
      const result = await rig.run(
        'Read format-test.txt and provide output in the same format as always.',
      );
    ...
    it('should maintain telemetry and logging format', async () => {
      ...
      const result = await rig.run(
        'Execute a simple operation to test telemetry compatibility.',
      );
    ...
    it('should maintain file operation result formats', async () => {
      ...
      const writeResult = await rig.run(
        'Create output.txt with "Test Output" and confirm the format.',
      );

      expect(writeResult).toBeTruthy();

      const toolLogs = rig.readToolLogs();
      const writeFileLog = toolLogs.find(
  ...
  describe('Integration Compatibility', () => {
    it('should work with existing IDE integrations', async () => {
      ...
      const result = await rig.run(
        'Test IDE integration compatibility by performing file operations.',
      );
    ...
    it('should maintain MCP server compatibility', async () => {
      ...
      const result = await rig.run(
        'Test MCP compatibility by using built-in tools normally.',
      );
    ...
    it('should preserve extension and plugin compatibility', async () => {
      ...
      const result = await rig.run(
        'Test extension compatibility with standard operations.',
      );
  ...
  describe('Performance Regression Tests', () => {
    it('should maintain or improve performance compared to Gemini-only', async () => {
      ...
      const result = await rig.run(
        'Read perf-test.txt and create a summary as efficiently as before.',
      );
    ...
    it('should not introduce memory leaks or resource issues', async () => {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const result = await rig.run(
          `Read resource-test-${i}.txt and process it efficiently.`,
        );
  ...
  describe('Regression Prevention', () => {
    it('should prevent common breaking changes', async () => {
      ...
      const successRate =
        toolLogs.length > 0 ? successfulTools / toolLogs.length : 0;
    ...
    it('should maintain API contract compatibility', async () => {
      ...
      const result = await rig.run(
        'Demonstrate that all core functionality works as expected.',
      );
