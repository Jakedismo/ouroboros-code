...
describe('Performance Benchmarking Tests', () => {
  ...
  describe('Basic Response Time Benchmarks', () => {
      ...
      for (const provider of providers) {
        ...
        try {
          const result = await rig.run(
            'Generate a simple greeting message explaining what you are.',
            ...args,
          );
          ...
          console.warn(
            `${provider} failed in simple text benchmark:`,
            error.message,
          );
          results.push({
            provider,
    ...
    it('should benchmark tool execution performance', async () => {
      ...
      for (const provider of providers) {
        ...
        rig.createFile(
          'bench-test.txt',
          'Benchmark test content for tool execution timing',
        );
        ...
        try {
          const result = await rig.run(
            'Read bench-test.txt and create a summary file called summary.txt.',
            ...args,
          );
          ...
          console.warn(
            `${provider} failed in tool execution benchmark:`,
            error.message,
          );
          results.push({
            provider,
  ...
  describe('Complex Task Benchmarks', () => {
    it('should benchmark multi-step workflow performance', async () => {
      ...
      for (const provider of providers) {
        ...
        try {
          const result = await rig.run(
            'Read all data files, analyze the JSON config, create a comprehensive report.txt with findings.',
            ...args,
          );
          ...
          const successRate =
            toolLogs.length > 0 ? (successfulTools / toolLogs.length) * 100 : 0;
          ...
          console.warn(
            `${provider} failed in workflow benchmark:`,
            error.message,
          );
          results.push({
            provider,
    ...
    it('should benchmark large file processing performance', async () => {
      ...
      for (const provider of providers) {
        ...
        try {
          const result = await rig.run(
            'Read large-file.txt and create a summary of its structure and content.',
            ...args,
          );
          ...
          console.warn(
            `${provider} failed in large file benchmark:`,
            error.message,
          );
          results.push({
            provider,
  ...
  describe('Concurrent Operation Benchmarks', () => {
    it('should benchmark concurrent task handling', async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        rig.createFile(
          `task-${i}.txt`,
          `Task ${i} content for concurrent processing`,
        );
      }
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const promise = rig
          .run(
            `Read task-${i + 1}.txt and create processed-${i + 1}.txt with analysis.`,
            ...args,
          )
          .then((result) => ({
            taskId: i + 1,
      ...
      console.log(
        `Tasks Completed: ${results.filter((r) => r.success).length}/5`,
      );

      results.forEach((result) => {
        console.log(
          `Task ${result.taskId} (${result.provider}): ${result.success ? 'SUCCESS' : 'FAILED'}`,
        );
    ...
    it('should benchmark provider failover performance', async () => {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        for (const testProvider of providers) {
          try {
            const args =
              testProvider === 'gemini' ? [] : ['--provider', testProvider];
            result = await rig.run(
              `Attempt ${attempt}: Create test-${attempt}.txt with success message.`,
              ...args,
            );
            ...
            console.warn(
              `Provider ${testProvider} failed in attempt ${attempt}:`,
              error.message,
            );
            continue;
          }
  ...
  describe('Memory and Resource Benchmarks', () => {
    it('should benchmark memory usage patterns', async () => {
      ...
      for (const provider of providers) {
        ...
        try {
          const result = await rig.run(
            'Read mem-test.txt, list directory, search for patterns, create output file.',
            ...args,
          );
          ...
          console.warn(
            `${provider} failed in memory benchmark:`,
            error.message,
          );
          results.push({
            provider,
      ...
      results
        ...
        .forEach((result) => {
          ...
          console.log(`  Duration: ${result.duration}ms`);
          console.log(
            `  RSS Delta: ${(result.memoryDelta.rss / 1024 / 1024).toFixed(2)} MB`,
          );
          console.log(
            `  Heap Used Delta: ${(result.memoryDelta.heapUsed / 1024 / 1024).toFixed(2)} MB`,
          );
          console.log(
            `  Heap Total Delta: ${(result.memoryDelta.heapTotal / 1024 / 1024).toFixed(2)} MB`,
          );
        });
      console.log('='.repeat(50));
      ...
      for (const provider of providers) {
        ...
        for (const complexity of complexityLevels) {
          ...
          for (let i = 1; i <= complexity; i++) {
            rig.createFile(
              `data-${i}.txt`,
              `Data file ${i} content for scalability test`,
            );
          }
          ...
          try {
            ...
            console.warn(
              `${provider} failed at complexity ${complexity}:`,
              error.message,
            );
            scalabilityResults.push({
              complexity,
  ...
  describe('Overall Performance Summary', () => {
    it('should generate comprehensive performance report', async () => {
      ...
      console.log(
        '█' + ' '.repeat(18) + 'PERFORMANCE SUMMARY' + ' '.repeat(19) + '█',
      );
      console.log('█'.repeat(60));
      ...
      for (const provider of providers) {
        ...
        try {
          const result = await rig.run(
            'Read test-data.txt, analyze it, create a report.txt, and list all files.',
            ...args,
          );
      ...
      overallResults.forEach((result, index) => {
        ...
        if (result.duration > 0) {
          console.log(`    Duration: ${result.duration}ms`);
          console.log(
            `    Tool Calls: ${result.toolCalls} (${result.successfulTools} successful)`,
          );
          console.log(`    Avg Tool Time: ${result.avgToolTime.toFixed(2)}ms`);
          console.log(`    Response Length: ${result.responseLength} chars`);
...
function calculatePerformanceScore(
  ...
  const reliabilityScore =
    toolCalls > 0 ? (successfulTools / toolCalls) * 30 : 0; // Up to 30 points
  ...
  return Math.max(
    ...
    Math.min(
      100,
      speedScore + reliabilityScore + qualityScore + efficiencyScore,
    ),
  );
}
