    ...
    describe('MCP Tool Discovery and Integration', () => {
        it('should discover MCP tools consistently across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                try {
                    const result = await rig.run('List available tools and check for MCP tools.', ...args);
        ...
        it('should handle MCP tool naming across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Show me the naming pattern for MCP tools.', ...args);
                    expect(result).toBeTruthy();
                    ...
                    console.warn(`MCP naming test failed for ${provider}:`, error.message);
                }
            }
        });
    });
    describe('MCP Tool Execution Across Providers', () => {
        it('should execute MCP tools with consistent behavior', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Use an MCP tool to perform a test operation.', ...args);
                    const endTime = Date.now();
                    const duration = endTime - startTime;
                    expect(result).toBeTruthy();
                    const toolLogs = rig.readToolLogs();
                    const mcpToolCalls = toolLogs.filter((log) => log.toolRequest.name.includes('.') ||
        ...
        it('should handle MCP tool parameters correctly across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run(`Test MCP tool parameter handling with this data: ${JSON.stringify(testData)}`, ...args);
                    expect(result).toBeTruthy();
                    // Should handle the complex parameter structure
                    expect(result.length).toBeGreaterThan(10);
                }
                catch (error) {
                    console.warn(`MCP parameter test failed for ${provider}:`, error.message);
                }
            }
        });
    });
    describe('MCP Tool Format Conversion', () => {
        it('should convert MCP tool schemas correctly for each provider', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Demonstrate MCP tool schema compatibility.', ...args);
                    expect(result).toBeTruthy();
                    // Each provider should handle schema conversion appropriately
                    const toolLogs = rig.readToolLogs();
                    // Should execute some tools successfully
                    const successfulTools = toolLogs.filter((log) => log.toolRequest.success);
                    expect(successfulTools.length).toBeGreaterThan(0);
                }
                catch (error) {
                    console.warn(`MCP schema test failed for ${provider}:`, error.message);
                }
            }
        });
        it('should handle MCP tool response formatting consistently', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Test MCP tool response formatting consistency.', ...args);
                    expect(result).toBeTruthy();
                    responseFormats.push({
                        provider,
                        responseLength: result.length,
                        containsStructuredData: result.includes('{') || result.includes('['),
                        containsMarkdown: result.includes('```') || result.includes('*'),
                        success: true,
                    });
                }
                catch (error) {
                    console.warn(`MCP response test failed for ${provider}:`, error.message);
                    responseFormats.push({
                        provider,
                        responseLength: 0,
    ...
    describe('MCP Tool Security and Validation', () => {
        it('should apply security assessments to MCP tools across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                try {
                    const result = await rig.run('Test MCP tool security assessment and validation.', ...args);
                    expect(result).toBeTruthy();
                    ...
                    console.warn(`MCP security test failed for ${provider}:`, error.message);
                }
            }
        });
        it('should validate MCP tool trust levels across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini'
                    ...
                    : ['--provider', provider, '--yolo'];
                try {
                    const result = await rig.run('Test MCP tool trust level validation.', ...args);
                    expect(result).toBeTruthy();
    ...
    describe('MCP Tool Error Handling', () => {
        it('should handle MCP tool failures gracefully across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Try to use a non-existent MCP tool to test error handling.', ...args);
                    expect(result).toBeTruthy();
                    // Should handle tool not found gracefully
                    expect(result.toLowerCase()).toMatch(/error|not found|invalid|unavailable/);
                }
                catch (error) {
        ...
        it('should handle MCP server connection failures across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Test MCP server connection failure handling.', ...args);
                    expect(result).toBeTruthy();
                    ...
                    console.warn(`MCP connection test had expected failure for ${provider}:`, error.message);
                }
            }
        });
    });
    describe('MCP Tool Performance Across Providers', () => {
        it('should benchmark MCP tool performance across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Perform MCP tool performance test.', ...args);
                    const endTime = Date.now();
                    const duration = endTime - startTime;
                    expect(result).toBeTruthy();
                    const toolLogs = rig.readToolLogs();
                    ...
                    console.warn(`MCP performance test failed for ${provider}:`, error.message);
                    performanceResults.push({
                        provider,
                        duration: -1,
                        toolCalls: 0,
    ...
    describe('MCP Integration Edge Cases', () => {
        it('should handle MCP tools with special characters in names', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Test MCP tools with special characters in tool names.', ...args);
                    expect(result).toBeTruthy();
                    // Should handle special characters gracefully
                    expect(result.length).toBeGreaterThan(5);
                }
                catch (error) {
                    console.warn(`MCP special chars test failed for ${provider}:`, error.message);
                }
            }
        });
        it('should handle MCP tools with large response payloads', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                try {
                    const result = await rig.run('Test MCP tools with large response payloads.', ...args);
                    expect(result).toBeTruthy();
                    // Should handle large responses appropriately
                    expect(result.length).toBeGreaterThan(0);
                }
                catch (error) {
                    console.warn(`MCP large response test failed for ${provider}:`, error.message);
                }
            }
        });
        ...
        it('should handle various MCP server configuration formats', async () => {
            ...
            for (const format of configFormats) {
                for (const provider of providers.slice(0, 1)) {
                    ...
                    const args = provider === 'gemini' ? [] : ['--provider', provider];
                    try {
                        const result = await rig.run(`Test MCP configuration format: ${format.name}`, ...args);
                        expect(result).toBeTruthy();
                        // Should handle the configuration format
                        expect(result.length).toBeGreaterThan(5);
                    }
                    catch (error) {
                        console.warn(`MCP config ${format.name} test failed for ${provider}:`, error.message);
                    }
                }
            }
        });
...
//# sourceMappingURL=mcp-multi-provider.test.js.map