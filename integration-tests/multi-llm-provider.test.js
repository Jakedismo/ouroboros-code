...
import { TestRig, validateModelOutput, createToolCallErrorMessage, printDebugInfo, } from './test-helper.js';
/**
...
describe('Multi-LLM Provider Integration Tests', () => {
    ...
    describe('Provider Switching Tests', () => {
        it('should default to Gemini provider', async () => {
            ...
            const result = await rig.run('What provider are you using? Use read_file to read a small test file.');
            expect(result).toBeTruthy();
            validateModelOutput(result, 'gemini', 'default-provider-test');
            const toolLogs = rig.readToolLogs();
            expect(toolLogs.length).toBeGreaterThan(0);
            expect(toolLogs.some((log) => ['read_file', 'ls'].includes(log.toolRequest.name))).toBe(true);
        });
        it('should switch to OpenAI provider with --provider flag', async () => {
            ...
            const result = await rig.run('Read the test.txt file and tell me what provider you are using.', '--provider', 'openai');
            expect(result).toBeTruthy();
        ...
        it('should switch to Anthropic provider with --provider flag', async () => {
            ...
            const result = await rig.run('Read the test.txt file and tell me what provider you are using.', '--provider', 'anthropic');
            expect(result).toBeTruthy();
            validateModelOutput(result, ['hello', 'anthropic'], 'anthropic-provider-test');
            // Verify tool was executed
            const toolLogs = rig.readToolLogs();
            expect(toolLogs.length).toBeGreaterThan(0);
            expect(toolLogs.some((log) => log.toolRequest.name === 'read_file')).toBe(true);
                ...
                for (const provider of providers) {
                    ...
                    switch (toolName) {
                        ...
                        case 'glob':
                            ...
                            break;
                        case 'grep':
                            rig.createFile('search.txt', 'This is a test\nAnother line\nTest pattern here');
                    ...
                    try {
                        ...
                        expect(result).toBeTruthy();
                        validateModelOutput(result, expectedContent, `${toolName}-${provider}-test`);
                        // Verify the specific tool was called
                        const toolLogs = rig.readToolLogs();
                        const toolCalled = toolLogs.some((log) => log.toolRequest.name === toolName);
                        if (!toolCalled) {
                            ...
                            throw new Error(createToolCallErrorMessage(toolName, allTools.map((t) => t.toolRequest.name), result));
                        }
                        // Verify tool execution was successful
                        const toolLog = toolLogs.find((log) => log.toolRequest.name === toolName);
                        expect(toolLog?.toolRequest.success).toBe(true);
                    }
                    catch (error) {
                        console.error(`Tool ${toolName} failed on provider ${provider}:`, error.message);
                        throw error;
                    }
                }
            });
    ...
    describe('Tool Confirmation Flow Tests', () => {
        it('should respect approval modes across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                const result = await rig.run('Create a file called dangerous.txt with content "This could be dangerous" using write_file tool.', ...args);
                expect(result).toBeTruthy();
                // Verify write_file was called (should succeed in YOLO mode)
                const toolLogs = rig.readToolLogs();
                const writeFileCalled = toolLogs.some((log) => log.toolRequest.name === 'write_file');
        ...
        it('should handle tool execution with security assessment', async () => {
            ...
            const result = await rig.run('List files and then create a backup directory using appropriate tools.');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
            // Should call safe operations (ls) and potentially dangerous operations (shell_command or write_file)
            const safeTools = toolLogs.filter((log) => ['ls', 'read_file', 'glob', 'grep'].includes(log.toolRequest.name));
            const potentiallyDangerousTools = toolLogs.filter((log) => ['shell_command', 'write_file'].includes(log.toolRequest.name));
    ...
    describe('Error Handling and Resilience Tests', () => {
        it('should handle network errors gracefully', async () => {
            ...
            const result = await rig.run('Try to fetch content from an invalid URL using web_fetch tool.');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
            const webFetchCall = toolLogs.find((log) => log.toolRequest.name === 'web_fetch');
            if (webFetchCall) {
        ...
        it('should handle tool parameter validation errors', async () => {
            ...
            const result = await rig.run('Try to read a file with an invalid path parameter using read_file tool.');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
    ...
    describe('Performance and Resource Tests', () => {
        it('should execute tools efficiently across providers', async () => {
            ...
            for (const provider of providers) {
                ...
                const args = provider === 'gemini' ? [] : ['--provider', provider];
                const result = await rig.run('Read perf-test.txt and list current directory files using appropriate tools.', ...args);
        ...
        it('should handle concurrent tool executions properly', async () => {
            ...
            const result = await rig.run('Read all three files (file1.txt, file2.txt, file3.txt) and summarize their contents.');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
            const readFileCalls = toolLogs.filter((log) => log.toolRequest.name === 'read_file');
    ...
    describe('Configuration and Settings Tests', () => {
        it('should respect provider-specific settings', async () => {
            ...
            const result = await rig.run('Create a simple test file using write_file tool.', '--provider', 'openai');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
    ...
    describe('Tool Statistics and Monitoring Tests', () => {
        it('should track tool usage statistics across providers', async () => {
            ...
            const result = await rig.run('Read stats-test.txt, list directory contents, and create a summary file.');
            expect(result).toBeTruthy();
            const toolLogs = rig.readToolLogs();
            expect(toolLogs.length).toBeGreaterThan(0);
        ...
        it('should provide comprehensive tool information', async () => {
            ...
            const result = await rig.run('Show me available tools and their capabilities.');
            expect(result).toBeTruthy();
...
//# sourceMappingURL=multi-llm-provider.test.js.map